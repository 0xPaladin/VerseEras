...
function gaussianRandom(mean = 0, stdev = 1) {
  const u = 1 - RNG.random(); // Converting [0,1) to (0,1]
  const v = RNG.random();
...
function getPoint(r) {
  const randomX =
    Math.pow(RNG.random(), parameters.randomnessPower) *
    (RNG.random() < 0.5 ? 1 : -1) *
    r *
...
function generateGalaxy() {
  ...
  for (let i = 0; i < parameters.count; i++) {
    ...
    let sid = [
      Math.floor((positions[i3] * GR) / (sectorScale * _r * 1000)),
      Math.floor((positions[i3 + 2] * GR) / (sectorScale * _r * 1000)),
    ]
