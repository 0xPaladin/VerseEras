const Animals = {
  air : ['pteranadon', 'condor', 'eagle/owl', 'hawk/falcon', 'crow/raven', 'heron/crane/stork', 'gull/waterbird', 'songbird/parrot','chicken/duck/goose','bee/hornet/wasp','locust/dragonfly/moth','gnat/mosquito/firefly'],
  earth : ['dinosaur/megafauna','elephant/mammoth','ox/rhinoceros','bear/ape/gorilla','deer/horse/camel','cat/lion/panther','dog/wolf/boar/pig','snake/lizard/armadillo','mouse/rat/weasel','ant/centipede/scorpion','snail/slug/worm','termite/tick/louse'],
  water: ['whale/narwhal','squid/octopus','dolphin/shark','alligator/crocodile','turtle','shrimp/crab/lobster','fish','frog/toad','eel/snake','clam/oyster/snail','jelly/anemone','insect/barnacle']
} 
...
const Magic = ['necromancy','evocation/destruction','conjuration/summoning','illusion/glamor','enchantment/artifice','transformation','warding/binding','restoration/healing','divination/scrying']
...
const GasGiantColors = HTMLColors.filter(c=> !(c.families.includes("black") || (c.families.includes("gray") && c.name.includes("GRAY")) || c.families.includes("white")))
const RockyColors = HTMLColors.filter(c=> {
  let rgb = c.rgb.slice(4).slice(0,-1).split(", ").map(Number).reduce((s,v)=>s+=v,0)
  return rgb < 600 && (c.families.includes("brown") || c.families.includes("tan")) 
})

const TerrainColors = {
  water : ["24,90,187","102,158,245","139,179,248","156,193,248"],
  vegetation : ["129,202,149","148,209,165","170,217,181","185,228,198","197,231,206"],
  shrub : ["242,233,218","244,240,228","251,248,241"],
  tundra : ["179,208,190","198,213,206","224,235,231"]
}

export {Animals,Oddities,Elements,Magic,GasGiantColors,HTMLColors,RockyColors,TerrainColors}